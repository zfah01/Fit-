    ...
    if ($ispost){
        ...
        if ($validated){
            ...
            if(check_if_email_exists($email)){
                ...
                $inserted = register_user($email, $passwordhash, $name, $dob, $height, $weight, $gender, $goals);
                if ($inserted) {
...
function register_user($email, $passwordhash, $name, $dob, $height, $weight, $gender, $goal){
    $server = "devweb2021.cis.strath.ac.uk";
    $username = "cs317madgroup22";
    $password = "hig4Leic8Red";
    $database = "cs317madgroup22";
    ...
    $connection = new mysqli($server, $username, $password, $database);
    $sql = "INSERT INTO user_profile (email, password, name, dob, height, weight, gender, goal) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"; // SQL with parameters
    $stmt = $connection->prepare($sql);
    $stmt->bind_param("ssssddii", $email, $passwordhash, $name, $dob, $height, $weight, $gender, $goal);
